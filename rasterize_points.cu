	...
	const bool debug)
{
  ...
  // channels: depth(1) + alpha(1) + normal(3) + median_depth(1) + distortion(1) + random_color(3) + surface_id(1) + depth_range(2)
  torch::Tensor out_others = torch::full({3+3+1+3+1+2, H, W}, 0.0, float_opts);
