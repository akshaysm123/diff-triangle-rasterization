    ...
    float* dL_dsigma_factor)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // forward kernel may have flipped the normal/offset sign, gradient must be consistent
        // points_xy_image[idx] is the triangle center in screen coords. Forward uses the screen space incenter
        ...
        // the offsets are calculated using the normals, the offset loss must be propageted
        // back to the normals
        ...
        // this is the inverse of the 90deg rotation 
        ...
        // the offsets are calculated using the p1, propagate the loss
