...
#else
#define RASTERIZER_IMPL_LAUNCH_LOG(expr) ((void)0)
#endif
...
void CudaRasterizer::Rasterizer::markVisible(
    ...
    bool* present)
{
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] markVisible: launch checkFrustum (blocks="
        << (P + 255) / 256 << ", threads=256, P=" << P << ")");
    ...
    bool debug)
{
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch FORWARD::preprocess (P=" << P << ")");
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch cub::DeviceScan::InclusiveSum (P=" << P << ")");
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch duplicateWithKeys (blocks="
        << (P + 255) / 256 << ", threads=256, P=" << P << ")");
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch cub::DeviceRadixSort::SortPairs (num_rendered="
        << num_rendered << ")");
    ...
    if (num_rendered > 0) {
        RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch identifyTileRanges (blocks="
            << (num_rendered + 255) / 256 << ", threads=256, num_rendered=" << num_rendered << ")");
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] forward: launch FORWARD::render (tile_grid="
        << tile_grid.x << "x" << tile_grid.y << ", block=" << block.x << "x" << block.y
    ...
    bool debug)
{
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] backward: launch BACKWARD::render (tile_grid="
        << tile_grid.x << "x" << tile_grid.y << ", block=" << block.x << "x" << block.y
        << ", R=" << R << ")");
    ...
    RASTERIZER_IMPL_LAUNCH_LOG("[rasterizer_impl] backward: launch BACKWARD::preprocess (P=" << P << ")");
